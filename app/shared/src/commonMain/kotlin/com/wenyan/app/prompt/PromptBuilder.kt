...
class PromptBuilder {
    ...
    fun buildProfileJson(profile: ProfileEntity?, target: TargetEntity?): String {
        val me = Json.obj()
        me.put("mbti", profile?.mbti ?: Json.NULL)
        me.put("score", profile?.score ?: Json.NULL)
        me.put("strengths", profile?.strengths ?: "")
        me.put("weaknesses", profile?.weaknesses ?: "")

        val targetObj = Json.obj()
        targetObj.put("codeName", target?.codeName ?: "")
        targetObj.put("mbti", target?.mbti ?: Json.NULL)
        targetObj.put("score", target?.score ?: Json.NULL)
        ...
        root.put("history", "")
        root.put("goal", "")
        val emotion = Json.obj()
        emotion.put("pain_point", "")
        emotion.put("intensity", Json.NULL)
    ...
    private fun parseTimeline(timeline: String?): JsonArray {
        if (timeline.isNullOrBlank()) return Json.arr()
        return try {
            Json.arr(timeline)
        } catch (e: Exception) {
            Json.arr()
