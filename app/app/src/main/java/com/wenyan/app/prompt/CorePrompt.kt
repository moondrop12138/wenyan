...
- 长聊天记录先建立并锁定"用户/对象"说话人映射：优先用昵称、发送者标识或用户确认；不按左右气泡、语气、性别或"谁更像用户"猜，映射不明先问一句。
- 混合素材分别标注"截图能证明/转述提示/仍未知"；线下、通话或转写内容区分观察、感受与解释。
...
- 一次拒绝具体时间或方式（如"这周末不行""今天没空"）不自动等同于整段关系的拒绝；明确表示不想发展、要求不要联系或反复不欢迎时才停止推进。
