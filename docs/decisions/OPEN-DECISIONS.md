...
| 2026-08 | O7 | 知识路由 BM25/精排生产切换 | 任务书前置：100–200 条真实 query 评测集，recall@3 提升 ≥15% 才保留精排层 | 已用 10 个子代理采集 Stack Exchange 公开匿名问答，构建 618 条中文真实素材评测集（route_eval_queries.json）。实测：contains precision@3=0.202 / recall@3=0.179；纯 BM25 precision@3=0.130 / recall@3=0.317（recall lift=+0.138 < 0.15，precision 下降 0.072）。按用户指示再加一次数据后仍未达标 → **停止，不切 BM25、不实现精排层，生产维持 contains 基线**；RouteEvaluator/Bm25Scorer 保留为后续评测工具 | 无（数据已完成） | 已按决策门关闭 | RESOLVED |
